...
def query_template_taxon(concept, property):
    query = (
        "SELECT ?concept ?conceptLabel ?peripheralConcept ?peripheralConceptLabel WHERE { "
...
def query_template_max_two(concept, property):
    query = (
        "SELECT ?concept ?conceptLabel ?peripheralConcept ?peripheralConceptLabel WHERE { "
        + f"?concept ((wdt:P279|wdt:P31)/(wdt:P279|wdt:P31)) wd:{concept}; "
        + f"wdt:{property} ?peripheralConcept. "
        + """SERVICE wikibase:label { bd:serviceParam wikibase:language "[AUTO_LANGUAGE],en". } """
        + "}"
    )
